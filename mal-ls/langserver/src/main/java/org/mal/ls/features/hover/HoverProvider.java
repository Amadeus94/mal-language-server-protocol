...
static public Set<HoverModel> fillItemsList(AST ast) {
        ...
        for (var i : ast.getCategories()) {
            ...
            for (var k : i.meta) {
               list.add(new HoverModel(k.getRange(), k.type.toString(), "meta", "the meta describes something about the category"));
            }
            for (var j : i.assets) {
        ...
        for (var i : ast.getAssociations()) {
            list.add(new HoverModel(i.getRange(),i.linkName.toString() , "association", "Any number of Asset1 instantiations can be connected to any number of Asset2 instantiations. Inline references from Asset1 to Asset2 use the name bar. Conversely, Asset2 refers to Asset1 with the name foo."));
        }
